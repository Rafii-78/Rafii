...
# Langsung mining blok oleh: Abim, Reoy, Panji, Ucok, Patih
for miner in ["Abim", "Reoy", "Panji", "Ucok", "Patih"]:
    bc.mine_pending_transactions(miner)

# Tambah transaksi: Lintar kirim ke semuanya
for name in ["Abim", "Reoy", "Panji", "Ucok", "Patih"]:
    bc.create_transaction("Lintar", name, 2)

# Miner Reoy menambang transaksi ini
...
app = Flask(__name__)
blockchain = bc  # bc = instance dari Blockchain class tadi
...
def mine():
    ...
    if not miner:
        return jsonify({'error': 'Nama penambang dibutuhkan'}), 400
    blockchain.mine_pending_transactions(miner)
    return jsonify({'message': f'{miner} berhasil mining dan dapat reward!'}), 200
...
def transaction():
    ...
    if not sender or not receiver or not amount:
        return jsonify({'error': 'Lengkapi data transaksi'}), 400
...
def balance(name):
    saldo = blockchain.get_balance(name)
    return jsonify({'user': name, 'balance': saldo}), 200
